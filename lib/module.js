...
function separateEnvModule() {
  ...
  const assignEnvVariables = (config, { isServer }) => {
...
function applyPrefix(definitionsToAdd) {
  return Object.entries(definitionsToAdd).reduce((newObject, [key, value]) => {
    ...
    newObject[`process.env.${key}`] = ['boolean', 'number'].includes(typeof value) ? value : JSON.stringify(value)
    return newObject
  }, {})
}

module.exports = separateEnvModule
